...
2. Run `make release VERSION=X.Y.Z`. It checks that the version matches `Cargo.toml` and every manifest, then tags and pushes `vX.Y.Z`. Tag with this command rather than `git tag`: the workflow does not compare the tag to `Cargo.toml` on its own.
