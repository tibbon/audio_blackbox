...
- **`make check-app-store` gates `fastlane mac metadata`.** The OpenAPI lint validates the metadata directory against the App Store Connect schema fastlane targets. `make verify` and the `make fl-metadata` flow both depend on it; if it fails, fix the metadata, do not bypass.
...
- **Tests**: integration tests live in `src/tests/`; the inline `mod tests` in `src/lib.rs` is for tiny smoke tests only (DOLL-170 deduped two that overlapped).
