...
impl PerformanceMonitor {
    ...
    fn get_metrics(&mut self) -> PerformanceMetrics {
        ...
        let cpu = 10.0 + (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)
            .unwrap().as_secs() % 80) as f32;
            
        // Simulate memory usage between 20-70%
        let mem = 20.0 + (std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)
            .unwrap().as_secs() % 50) as f32;
