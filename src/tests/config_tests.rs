...
fn test_config_file_creation() {
    ...
    let config = AppConfig::default();
    config.create_config_file(config_path.to_str().unwrap()).unwrap();
...
fn test_config_env_vars() {
    ...
    temp_env::with_vars([
        ...
        assert_eq!(config.get_audio_channels(), "0,1,2", "Config should load audio_channels from file");
        assert_eq!(config.get_debug(), false, "Config should load debug from file");
    });
}
...
fn test_config_env_vars_precedence() {
    ...
    temp_env::with_vars([
        ...
        println!("  BLACKBOX_AUDIO_CHANNELS: {:?}", env::var("BLACKBOX_AUDIO_CHANNELS"));
        ...
        assert_eq!(config.get_audio_channels(), "3,4,5", "Environment variable should override config file");
        assert_eq!(config.get_debug(), true, "Environment variable should override config file");
        assert_eq!(config.get_duration(), 120, "Environment variable should override config file");
        assert_eq!(config.get_output_mode(), "split", "Environment variable should override config file");
        assert_eq!(config.get_silence_threshold(), 0.001, "Environment variable should override config file");
        assert_eq!(config.get_continuous_mode(), true, "Environment variable should override config file");
        assert_eq!(config.get_recording_cadence(), 600, "Environment variable should override config file");
        assert_eq!(config.get_output_dir(), "/tmp/test_output", "Environment variable should override config file");
        assert_eq!(config.get_performance_logging(), true, "Environment variable should override config file");
    });
}
...
fn test_config_invalid_env_vars() {
    ...
    temp_env::with_vars([
        ...
        assert_eq!(config.get_audio_channels(), "3,4,5", "Valid channel spec should be used");
        assert_eq!(config.get_debug(), DEFAULT_DEBUG, "Invalid debug should fall back to default");
        assert_eq!(config.get_duration(), DEFAULT_DURATION, "Invalid duration should fall back to default");
        assert_eq!(config.get_output_mode(), DEFAULT_OUTPUT_MODE, "Default output mode should be used");
        assert_eq!(config.get_silence_threshold(), DEFAULT_SILENCE_THRESHOLD, "Invalid threshold should fall back to default");
        assert_eq!(config.get_continuous_mode(), DEFAULT_CONTINUOUS_MODE, "Invalid continuous mode should fall back to default");
        assert_eq!(config.get_recording_cadence(), DEFAULT_RECORDING_CADENCE, "Invalid cadence should fall back to default");
        assert_eq!(config.get_output_dir(), DEFAULT_OUTPUT_DIR, "Default output dir should be used");
        assert_eq!(config.get_performance_logging(), DEFAULT_PERFORMANCE_LOGGING, "Invalid performance logging should fall back to default");
    });
}
